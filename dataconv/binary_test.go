...
func TestEncodeString(t *testing.T) {
	str := "Hello world!"
	b := NewBinaryEncoder()
	assert.Equal(t, b.Encode(str), nil)
	data := b.Bytes()
	assert.Equal(t, data, []byte{0, 0, 0, 12, 72, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100, 33})
...
func TestEncodeUint(t *testing.T) {
	b := NewBinaryEncoder()
	assert.Equal(t, b.Encode(uint8(1)), nil)
	assert.Equal(t, b.Encode(uint16(1245)), nil)
	assert.Equal(t, b.Encode(uint32(1245678)), nil)
	assert.Equal(t, b.Encode(uint64(124567891011)), nil)
	assert.Equal(t, b.Encode([]byte("test")), nil)
	assert.Equal(t, b.Encode(12.56), nil)
	assert.Equal(t, b.Encode(float32(3.14)), nil)
	data := b.Bytes()
	...
	err := d.Decode(&tuint8)
	assert.Equal(t, err, nil)
	assert.Equal(t, tuint8, uint8(1))

	var tuint16 uint16

	err = d.Decode(&tuint16)
	assert.Equal(t, err, nil)
	assert.Equal(t, tuint16, uint16(1245))

	var tuint32 uint32

	err = d.Decode(&tuint32)
	assert.Equal(t, err, nil)
	assert.Equal(t, tuint32, uint32(1245678))

	var tuint64 uint64

	err = d.Decode(&tuint64)
	assert.Equal(t, err, nil)
	assert.Equal(t, tuint64, uint64(124567891011))

	var tbytes []byte

	err = d.Decode(&tbytes)
	assert.Equal(t, err, nil)
	assert.Equal(t, tbytes, []byte("test"))

	var tfloat64 float64

	err = d.Decode(&tfloat64)
	assert.Equal(t, err, nil)
	assert.Equal(t, tfloat64, float64(12.56))

	var tfloat32 float32

	err = d.Decode(&tfloat32)
	assert.Equal(t, err, nil)
	assert.Equal(t, tfloat32, float32(3.14))
}
